...
public class Home extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(mobileapptaylors.example.MAD_assignment1.R.layout.activity_home);

        Toolbar toolbar =findViewById(mobileapptaylors.example.MAD_assignment1.R.id.toolbar);
        setSupportActionBar(toolbar);

        drawer = findViewById(mobileapptaylors.example.MAD_assignment1.R.id.drawer_layout);
        NavigationView navigationView = findViewById(mobileapptaylors.example.MAD_assignment1.R.id.nav_view);
        navigationView.setNavigationItemSelectedListener(this);

        ActionBarDrawerToggle toggle = new ActionBarDrawerToggle(this, drawer, toolbar,
                mobileapptaylors.example.MAD_assignment1.R.string.navigation_drawer_open, mobileapptaylors.example.MAD_assignment1.R.string.navigation_drawer_close);
        ...
        RecyclerView recList = (RecyclerView) findViewById(mobileapptaylors.example.MAD_assignment1.R.id.next_slot);
        recList.setHasFixedSize(true);
        LinearLayoutManager llm = new LinearLayoutManager(this);
        llm.setOrientation(LinearLayoutManager.VERTICAL);
        recList.setLayoutManager(llm);

        TextView welcome = (TextView) findViewById(mobileapptaylors.example.MAD_assignment1.R.id.welcome_greeting);
        TextView time_now = (TextView) findViewById(mobileapptaylors.example.MAD_assignment1.R.id.current_time);
        TextView total_slots = (TextView) findViewById(mobileapptaylors.example.MAD_assignment1.R.id.slots_total);
...
//                System.out.println("date of slots"+dateformat.format(date));
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()){
            case mobileapptaylors.example.MAD_assignment1.R.id.nav_home:
                Intent intent_home = new Intent(this, Home.class);
                this.startActivity(intent_home);
                break;
            case mobileapptaylors.example.MAD_assignment1.R.id.nav_addslot:
                Intent intent = new Intent(this, AddSlots.class);
                this.startActivity(intent);
                break;
            case mobileapptaylors.example.MAD_assignment1.R.id.nav_viewslot:
                Intent intent_viewSlots = new Intent(this, ViewSlots.class);
                this.startActivity(intent_viewSlots);
                break;

            case mobileapptaylors.example.MAD_assignment1.R.id.nav_historyslot:
                Intent intent_historySlot = new Intent(this, HistorySlots.class);
                this.startActivity(intent_historySlot);
                break;

            case mobileapptaylors.example.MAD_assignment1.R.id.nav_announcement:
                Intent intent_announcement = new Intent(this, Announcement.class);
                this.startActivity(intent_announcement);
                break;

            case mobileapptaylors.example.MAD_assignment1.R.id.nav_aboutus:
//                getSupportFragmentManager().beginTransaction().replace(mobileapptaylors.example.MAD_assignment1.R.id.fragment_container,
//                        new AboutusFragment()).commit();
                Intent intent_aboutUs = new Intent(this, AboutUs.class);
                this.startActivity(intent_aboutUs);
                break;

            case mobileapptaylors.example.MAD_assignment1.R.id.nav_help:
//                getSupportFragmentManager().beginTransaction().replace(mobileapptaylors.example.MAD_assignment1.R.id.fragment_container,
